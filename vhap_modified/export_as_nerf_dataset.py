...
class NeRFDatasetWriter:
    def __init__(self, cfg_data: DataConfig, tgt_folder: Path, subset:Optional[str]=None, scale_factor: Optional[float]=None, background_color: Optional[str]=None):
        ...
        self.dataloader = DataLoader(dataset, shuffle=False, batch_size=None, collate_fn=lambda x: x, num_workers=0)  # 0 on Windows (can't pickle lambda)
...
class TrackedFLAMEDatasetWriter:
    ...
    def write(self):
        ...
        for i, frame in tqdm(enumerate(self.db['frames']), total=len(self.db['frames'])):
            ...
            ti_orig = frame['timestep_index_original']  # original (pre-subset) frame number
            ti = frame['timestep_index']  # use ti when saving files
            # When a `subset` (e.g. ts3) is used, tracked_flame_params are saved COMPACTED to the
            # subset length, so they must be indexed by the compacted `ti`, not the original index.
            ti_param = ti if self.flame_params['translation'].shape[0] == len(self.db['timestep_indices']) else ti_orig
            ...
            if self.mode == 'mesh':
                ...
                if not saved[ti]:
                    worker_args.append([self.tgt_folder, frame['exp_path'], self.flame_params['expr'][ti_param], frame['mesh_path'], verts[ti_param], self.flame_model.faces])
                    saved[ti] = True
                    func = self.write_expr_and_mesh
            elif self.mode == 'param':
                frame['flame_param_path'] = f"flame_param/{ti:05d}.npz"
                if not saved[ti]:
                    worker_args.append([self.tgt_folder, frame['flame_param_path'], self.flame_params, ti_param])
                    saved[ti] = True
...
class MaskFromFLAME:
    def __init__(self, cfg_data: DataConfig, cfg_model: ModelConfig, tgt_folder, background_color: str) -> None:
        ...
        self.dataloader = DataLoader(dataset, shuffle=False, batch_size=None, collate_fn=None, num_workers=0)  # 0 on Windows (can't pickle lambda)
