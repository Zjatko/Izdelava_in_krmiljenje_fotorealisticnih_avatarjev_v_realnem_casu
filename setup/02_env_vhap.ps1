# 02_env_vhap.ps1
# Creates the `VHAP` conda env (separate from gaussian-avatars -- it uses a patched nvdiffrast).
...
# ---------------------------------------------------------------------------
# Helper: enter the VS2022 x64 developer environment (PowerShell vcvars64.bat).
# ---------------------------------------------------------------------------
...
# --- 6. VHAP itself ---
# --no-build-isolation so CUDA deps (pytorch3d, dlib) see the installed torch at build time.
# But that means VHAP's own build backend must already be installed -> add hatchling/editables.
# NOTE: this COMPILES pytorch3d's CUDA kernels -- can take 15-40 min.
Set-Location "$ROOT\VHAP"
pip install hatchling editables
pip install -e . --no-build-isolation

# --- 7. Patched nvdiffrast (backface-culling fork required by VHAP) ---
# --no-build-isolation so the CUDA extension can see torch at build time.
pip install nvdiffrast@git+https://github.com/ShenhanQian/nvdiffrast@backface-culling --force-reinstall --no-build-isolation

# re-assert numpy<2 in case a dependency bumped it
pip install "numpy<2"

Write-Host "`nVHAP env ready. Next: place FLAME files (03_flame_checklist.md), then 05_track_my_video.ps1" -ForegroundColor Green
